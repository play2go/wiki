...
Выбираем одну из них и переходим на следующий этап:
...
1. Выбираем в поле для выбора дополнительного ПО: `OpenVPN`
2. Покупаем сервер, и после его установки заходим на него с помощью гайда [`Как подключиться к VDS по SSH`](/vds/ssh), для новичков рекомендую Termius. Логин, пароль и айпи сервера должны быть на вашей почте.
3. Далее переходим в SFTP. В **Termius** для такого есть специальная вкладка, а для других советую использовать SFTP клиент **WinSCP**. Подключение в нем простое, как порт используйте 22.
4. При входе в SFTP там должен быть файл `название.ovpn`. Перетаскиваете в любую папку вашего компьютера.
...
6. Чтобы зайти на сам сервер, нам нужно импортировать файл. Кликаем на кнопку плюс, выбираем вкладку `Import file` и закидываем скачанный файл с сервера.
7. Подключаемся и пользуемся **своим ВПН**. Можете похвастаться друзьям, какой вы молодец :)
...
1. Выбираем в поле для выбора дополнительного ПО: `WireGuard`
2. Покупаем сервер, и после его установки заходим на него с помощью гайда [`Как подключиться к VDS по SSH`](/vds/ssh), для новичков рекомендую Termius. Логин, пароль и айпи сервера должны быть на вашей почте.
3. Далее переходим в SFTP. В **Termius** для такого есть специальная вкладка, а для других советую использовать SFTP клиент **WinSCP**. Подключение в нем простое, как порт используйте 22.
4. Переходим в папку `/etc/wireguard/client/`, там должен быть файл `client.conf`. Перетаскиваете в любую папку вашего компьютера.
