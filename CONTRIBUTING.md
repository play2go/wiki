...
4. Markdown файлы должны проходить проверку [линтером](https://marketplace.visualstudio.com/items?itemName=DavidAnson.vscode-markdownlint) (если вы используете [консольную версию](https://github.com/igorshubovych/markdownlint-cli), то не забудьте указать путь к [файлу конфигурации](https://github.com/play2go/wiki/blob/main/.markdownlint.yml)), а также статья должна быть заранее просмотрена через `npm run dev` на предмет явных дефектов, которые могут повлечь ошибки билда (см. [Запуск](https://github.com/play2go/wiki#%D0%B7%D0%B0%D0%BF%D1%83%D1%81%D0%BA));
